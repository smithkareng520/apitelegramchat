...
async def get_file_path(file_id: str) -> str:import os
import time
import random
import aiohttp
import asyncio
...
async def parse_audio_file(file_path: str, file_name: str) -> str:
    """解析音频文件，支持 WAV 和 MP3，OGG 需要转换"""
    try:
        print(f"[DEBUG] 解析音频文件: {file_path}")
        if file_name.endswith(".ogg"):
            # 转换 OGG 到 WAV
            audio = AudioSegment.from_ogg(file_path)
            wav_path = file_path.replace(".ogg", ".wav")
            audio.export(wav_path, format="wav")
            os.remove(file_path)  # 删除原始 OGG 文件
            file_path = wav_path
        elif not (file_name.endswith(".wav") or file_name.endswith(".mp3")):
            return None

        # 这里我们不直接转录，而是返回文件路径供 Gemini 处理
        return file_path
    except Exception as e:
        print(f"[ERROR] 音频解析失败: {str(e)}")
        return None
...
async def parse_file(file_id: str, file_name: str) -> str:
    ...
    try:
        ...
        if file_name.endswith(".txt"):
            ...
            content = await parse_audio_file(file_path, file_name)
        else:
