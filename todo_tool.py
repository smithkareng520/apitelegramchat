...
1. 给 AI agent 一个轻量、持久的任务管理能力——支持新增、列表、完成、
   反完成、删除、清空、编辑、改优先级。
2. 数据按 chat 隔离，落在 ./workspace/{chat_id}/todos.json，复用既有
...
def _find_todo(todos: list, todo_id: str) -> tuple[int, dict] | None:
    """返回 (index, todo) 或 None。同时支持按 seq 序号（字符串）查找。"""
    if not todo_id:
        return None
...
async def _mutate(chat_id: int, fn) -> dict:
    """
    在 workspace 锁保护下：从 R2 同步 → 加载 → 调用 fn(store) → 保存 → 异步回传 R2。
...
def render_todo_card(payload: dict, max_items: int = 50) -> str:
    ...
    if action == "add":
        t = payload.get("todo", {})
        return (
            f"<p>➕ <b>已新增待办</b> <code>#{t.get('seq')}</code></p>"
    ...
    if action in ("done", "undone", "toggle"):
        ...
        return (
            f"<p>{icon} <b>{verb}</b> <code>#{t.get('seq')}</code></p>"
            f"<p>{_priority_badge(t)} {_esc(t.get('title'))} {_tag_chips(t)}</p>"
    ...
    if action == "delete":
        t = payload.get("todo", {})
        return (
            f"<p>🗑️ <b>已删除</b> <code>#{t.get('seq')}</code></p>"
    ...
    if action == "edit":
        t = payload.get("todo", {})
        return (
            f"<p>📝 <b>已编辑</b> <code>#{t.get('seq')}</b></p>"
            f"<p>{_priority_badge(t)} {_esc(t.get('title'))} {_tag_chips(t)}</p>"
...
def _render_todo_item(t: dict) -> str:
    ...
    parts = [f"{status} {badge} {seq} {title_html}"]
    if tags:
        parts.append(tags)
    line = " ".join(parts)
    if note_html:
...
def build_todo_keyboard(payload: dict, max_buttons: int = 12) -> dict | None:
    ...
    for t in todos:
        ...
        # 按钮文字带序号，方便用户对位
        rows.append([
            {"text": f"{toggle_text} #{seq}", "callback_data": toggle_cb[:64]},
            {"text": f"✕ #{seq}", "callback_data": del_cb[:64]},
        ])
...
TODO_TOOL = {
    ...
    "function": {
        ...
        "input_examples": [
            ...
            {"action": "done", "todo_id": "3"},
            {"action": "delete", "todo_id": "3"},
            {"action": "edit", "todo_id": "5", "title": "改后的标题", "priority": "low"},
