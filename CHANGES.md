...
# fetch_url 长内容表格丢失修复（v4）— 预算感知压缩

## 背景（真实 bug：zh.wikipedia 可塑性記憶 各話列表丢失）

用户反馈 fetch `https://zh.wikipedia.org/wiki/可塑性記憶` 时，每集列表
（話數/日文標題/中文標題/分鏡/演出/作畫監督 表格）不再出现在结果里。
逐层排查定位根因：

- `extract_body_blocks` 层表格提取正常（114 块，每话表完整，第 82 块）；
- 组装链（锚定/排序/轮播/interleave）全部无损（117 块）；
- **丢失发生在 `_truncate_blocks`**：正文共 25290 字符，而
  `FETCH_BODY_MAX_LEN=11000`，截断只保留前 69 块——每话表在 13806 偏移
  处，被整块丢弃。

为什么"以前可以"：旧版 trafilatura 纯文本输出没有 URL 膨胀，同样页面
只有约 7K 字符，全部装得下；富 HTML 版本里维基百科内链 URL 是百分号
编码（一个 CJK 字符展开为 9 个 ASCII 字符），实测该页 118 条内链吃掉
10609 字符（42% 预算）。

## 修复（两管齐下）

1. **正文预算 11000 → 13400**：用满 FETCH_RICH_MAX_LEN=14000 窗口
...
2. **`_demote_same_origin_links`（预算感知压缩，新增）**：仅当正文超出
   ...
   不变（真实页验证：6831 字符完全一致），信息几乎无损。

效果（真实页）：25290 → 压缩后 14681，保留 113/117 块；4 张表全部
保留，13 集标题（日文+中文）完整提取，剩余链接 37 条全为跨域官方
站点（官网/角川/ANN 等）；最终结果 9895 字符。

## 测试

- 新增 `TestBudgetCompaction`（3 项）+ 预算不变式（1 项）：
  - 同源降级/跨域保留/嵌套格式保真/可见文本无损；
  - 预算内不降级（保真优先）；
  - 回归复现：链接膨胀 + 每话表在预算之外 → 表格存活、跨域链接保留、
    页头来源链接不受影响（monkeypatch FETCH_BODY_MAX_LEN）；
  - FETCH_RICH_MAX_LEN > FETCH_BODY_MAX_LEN + 400 不变式。
- 全套 64 项单测 + `test_consumers.py` 全绿；真实页面走
  `execute_fetch_url` 真实入口（`_build_rich_fetch_payload`）验证
  13 集完整提取。
- 另：`wikipedia` 工具走 MediaWiki API 返回纯文本摘要（无表格），
