...
# ===================== 已废弃的 _validate_edit（保留以防外部引用） =====================
async def _validate_edit(chat_id: int, path: str, old_str: str, start_line: int, end_line: int, allow_multi: bool, use_regex: bool):
    """
    ...
    state = get_editor_file_state(chat_id, path)
    if not state:
        return False, f"File '{path}' has not been viewed. Please call 'view' first.", None, None, None, None
    current_mtime = local_path.stat().st_mtime
    if current_mtime != state['mtime']:
        with open(local_path, 'r', encoding='utf-8') as f:
            current_content = f.read()
        if current_content == state['content']:
            set_editor_file_state(chat_id, path, current_content, current_mtime)
        else:
            return False, f"File '{path}' has been modified externally since last view. Please re-view it.", None, None, None, None
    with open(local_path, 'r', encoding='utf-8') as f:
        full_content = f.read()
    ...
    if start_line > total_lines:
        return False, f"start_line {start_line} exceeds total lines {total_lines}", None, None, None, None
    ...
    if use_regex:
        ...
        count = slice_content.count(old_str)
        match_positions = []
        if count > 0:
            start_idx = 0
            while True:
                idx = slice_content.find(old_str, start_idx)
                if idx == -1:
                    break
                match_positions.append((idx, idx + len(old_str)))
                start_idx = idx + 1
    if count == 0:
        stripped_old = old_str.strip()
        if stripped_old != old_str and not use_regex:
            count2 = slice_content.count(stripped_old)
            if count2 > 0:
                return True, "Auto-stripped whitespace", slice_content, match_positions, stripped_old, full_content
        return False, f"No match found for replacement text in lines {start_line}-{end_line}.", None, None, None, None
    if not allow_multi and count > 1:
        ...
        for i, line in enumerate(lines[start_line-1:end_line], start=start_line):
            if use_regex:
                ...
                if old_str in line:
                    line_numbers.append(i)
        return False, (f"Found {count} matches in lines {start_line}-{end_line}. "
                       f"Matches appear around lines: {line_numbers[:10]}{'...' if len(line_numbers)>10 else ''}. "
                       f"Please set 'allow_multi'=True to replace all, or set 'occurrence' to a specific number (1-{count})."), None, None, None, None
    return True, "Validation passed", slice_content, match_positions, None, full_content
...
async def execute_web_search(query: str, num_results: int = 5) -> str:
    # 检查缓存
    cached = get_search_cache(query, num_results)
    if cached is not None:
        logger.debug(f"Search cache hit for {query} (num_results={num_results})")
        return cached
    ...
    set_search_cache(query, result, num_results)
    return result
