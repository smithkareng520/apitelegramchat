...
# 助手消息的 content 字段」通过 sendRichMessage 交付给用户。无需任何参数
# ——系统会原样发送调用本工具的那条消息的 content（即 agent 轮次的最后
# 消息，仅 content 字段本身，不含 reasoning 等其他字段），模型不必把正
# 文重复写进参数。静默回合没有任何兑底：不调用本工具，本轮就不会有任何
# 内容送达用户。草稿开启（/show on）时系统会自动发送最终回复，本工具
# 不进入工具面，且历史中的调用痕迹也会从出站上下文拔除（见
...
DELIVER_REPLY_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "（即本轮最后一条助手内容的 content 字段本身，不含其他内容）作为一条永久"
            "富文本消息（Telegram Rich HTML）通过 sendRichMessage 直接发送给用户，"
            "不经过草稿。无需任何参数——系统发送的就是你当前消息的正文本身，因此正确"
            "用法是：先把完整、自包含的最终回复直接写成消息正文，再在同一条消息里调用"
            "本工具（每轮最多一次）。重要：在正文中用文字\"提到\"或\"声称已使用\"本工具"
            "不会产生任何效果——只有通过标准 tool_calls API 机制真正发起调用，系统才会"
            "发送消息；写了正文却没调用本工具，用户什么也收不到。静默模式下你的流式"
            "输出与最终回复都不会自动送达用户，系统也不会代替你兑底发送——若本轮需要"
            "用户看到结论，必须用本工具交付；若整轮无需用户知晓（例如后台巡检无值得"
            "汇报的内容），则不要调用，保持静默，什么都不要发送。"
        ),
        "parameters": {
            "type": "object",
            "properties": {},
