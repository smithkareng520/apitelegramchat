...
async def memory_manage(context: MCPRequestContext, args: JsonObject) -> str:
    ...
    return await invoke(execute_memory, context.chat_id, namespace=context.scope, **args)
...
MUTATION_SPECS: tuple[ToolSpec, ...] = (
    ToolSpec("memory.manage", "Manage file memory", "Manage persistent private memory files under /memories. Supports just-in-time view and safe text editing. Explicit deployment opt-in required.", object_schema({"command": {"type": "string", "enum": ["view", "create", "str_replace", "insert", "delete", "rename"]}, "path": text_field("Memory path under /memories."), "view_range": {"type": "array", "items": {"type": "integer"}, "minItems": 2, "maxItems": 2}, "file_text": text_field("Complete text for create."), "old_str": text_field("Exact unique text for str_replace."), "new_str": text_field("Replacement text; omit to delete old_str."), "insert_line": int_field("Insert after 1-based line; use 0 for the start.", 0), "insert_text": text_field("Text to insert."), "old_path": text_field("Existing memory path for rename."), "new_path": text_field("New memory path for rename.")}, ("command",)), memory_manage),
