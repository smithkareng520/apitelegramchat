...
READ_ONLY_SPECS: tuple[ToolSpec, ...] = (
    ...
    ToolSpec("search.weather", "Weather", "Retrieve current weather and forecast for a city (data source: wttr.in). The 'current' block is the most recent station OBSERVATION — its obs_time is the local timestamp at which the upstream station actually took the reading, NOT the moment this tool was called, and may lag real-time by 15-60 minutes. Also returns 'hourly' (up to 24 hours of forecast) and 'daily' (up to 5 days). Use unit='c' for Celsius (default) or 'f' for Fahrenheit.", object_schema({"city": text_field("City or location.", 1), "unit": {"type": "string", "enum": ["c", "f"]}, "hours": int_field("Forecast hours.", 1, 24)}, ("city",)), weather),
