...
"""
    ...
    # 这里的 details_html 只是工具调用气泡里的折叠预览，两者共用 render_todo_card。
        ...
        if action == "add":
            t = payload.get("todo", {})
            summary = f"➕ 新增 #{t.get('seq', '?')} {t.get('title', '')[:30]}"
            details_html = render_todo_card(payload)
            return summary, details_html
        if action in ("done", "undone", "toggle"):
            t = payload.get("todo", {})
            icon = "✅" if t.get("done") else "↩️"
            summary = f"{icon} #{t.get('seq', '?')} {t.get('title', '')[:30]}"
            details_html = render_todo_card(payload)
            return summary, details_html
        if action == "delete":
            t = payload.get("todo", {})
            summary = f"🗑️ 删除 #{t.get('seq', '?')} {t.get('title', '')[:30]}"
            details_html = render_todo_card(payload)
            return summary, details_html
        if action == "clear":
        ...
        if action == "edit":
            t = payload.get("todo", {})
            summary = f"📝 编辑 #{t.get('seq', '?')} {t.get('title', '')[:30]}"
            details_html = render_todo_card(payload)
            return summary, details_html
        summary = "📋 待办操作"
        details_html = render_todo_card(payload)
