...
# Bash 的实时草稿预览是一个独立的、小型终端视口：它不复用完整 stdout，
# 因此即使命令输出了大量数据、超长单行或进度条，也不会把 rich draft 撑爆。
_BASH_PROGRESS_PREVIEW_MAX_LINES = 20
_BASH_PROGRESS_PREVIEW_MAX_LINE_CHARS = 240


@dataclass(frozen=True)
class BashProgressSnapshot:
    """A bounded, presentation-only view of command output for the live draft."""
...
class BashPreviewBuffer:
    """Maintain the last visible terminal lines without retaining full stdout.

    The command result remains separately collected for the model.  This buffer only
    feeds the Telegram draft and deliberately handles the terminal controls that make
    a character-tail preview look broken: ANSI styling, carriage-return progress bars,
    backspaces, tabs and arbitrarily long lines.
    """

    def __init__(
        self,
        *,
        max_lines: int = _BASH_PROGRESS_PREVIEW_MAX_LINES,
        max_line_chars: int = _BASH_PROGRESS_PREVIEW_MAX_LINE_CHARS,
        hidden_line_prefixes: tuple[str, ...] = (),
    ):
        self.max_lines = max(1, max_lines)
        self.max_line_chars = max(16, max_line_chars)
        self.hidden_line_prefixes = tuple(prefix for prefix in hidden_line_prefixes if prefix)
        self._completed: Deque[tuple[int, str]] = deque(maxlen=self.max_lines)
        self._line_number = 1
        self._current: list[str] = []
        self._current_truncated = False
        self._has_current_activity = False
        self._ansi_state = ""
        self._carriage_return_pending = False

    def _current_text(self) -> str:
        text = "".join(self._current)
        return text + ("…" if self._current_truncated else "")

    def _is_hidden_internal_line(self, text: str) -> bool:
        # While a marker is being received across chunks, hide the partial marker too.
        # It is generated by this module and cannot be meaningful user output.
        return any(text.startswith(prefix) or prefix.startswith(text) for prefix in self.hidden_line_prefixes)

    def _commit_current_line(self) -> None:
        text = self._current_text()
        # 命令封装器会额外输出一个换行后再写内部 marker；空行不占用
        # 用户可见的 20 行窗口，避免内部协议噪声挤掉真实输出。
        if self._has_current_activity and not self._is_hidden_internal_line(text):
            self._completed.append((self._line_number, text))
            self._line_number += 1
        self._current.clear()
        self._current_truncated = False
        self._has_current_activity = False

    def _append_visible_char(self, char: str) -> None:
        if self._carriage_return_pending:
            self._carriage_return_pending = False
            if char == "\n":
                # Preserve ordinary CRLF as one completed line rather than treating it
                # as an in-place progress redraw.
                self._commit_current_line()
                return
            # A bare carriage return followed by text is a terminal overwrite (for
            # example a progress bar), so discard the earlier version of that row.
            self._current.clear()
            self._current_truncated = False
            self._has_current_activity = False
        if char == "\n":
            ...
            return
        if char == "\r":
            # Delay the decision until the next character so CRLF stays a real newline.
            self._carriage_return_pending = True
            return
        if char == "\b":
            if self._current:
                self._current.pop()
            self._has_current_activity = True
            return
        if char == "\t":
            for _ in range(4):
                self._append_visible_char(" ")
            return
        if ord(char) < 32 or ord(char) == 127:
            return

        self._has_current_activity = True
        if len(self._current) < self.max_line_chars:
            self._current.append(char)
        else:
            self._current_truncated = True

    def _consume_char(self, char: str) -> None:
        # Strip CSI and OSC terminal escape sequences incrementally, including the case
        # where an escape sequence is split between two stdout chunks.
        if self._ansi_state == "esc":
            if char == "[":
            ...
            return
        if self._ansi_state == "csi":
            if "@" <= char <= "~":
                self._ansi_state = ""
            return
        if self._ansi_state == "osc":
            if char == "\a":
                self._ansi_state = ""
            elif char == "\x1b":
                self._ansi_state = "osc_esc"
            return
        if self._ansi_state == "osc_esc":
            if char == "\\" or char == "\a":
                self._ansi_state = ""
            elif char != "\x1b":
                self._ansi_state = "osc"
    ...
    def snapshot(self) -> BashProgressSnapshot | None:
        visible = list(self._completed)
        current = self._current_text()
        if self._has_current_activity and not self._is_hidden_internal_line(current):
            visible.append((self._line_number, current))
        if len(visible) > self.max_lines:
            visible = visible[-self.max_lines:]
        if not visible:
            return None
        first_line = visible[0][0]
        total_lines = visible[-1][0]
        return BashProgressSnapshot(
            text="\n".join(text for _, text in visible),
            first_line=first_line,
            total_lines=total_lines,
            hidden_lines=max(0, first_line - 1),
        )
    ...
    async def execute(self, command: str, timeout: int = SANDBOX_TIMEOUT_SEC, progress_callback=None) -> str:
        ...
        async with lock:
            ...
            try:
                ...
                output_parts = []
                # 草稿预览必须与完整结果分离：完整 output_parts 仍返回给模型，
                # preview_buffer 则始终只保存可展示的最近 20 个视觉行。
                preview_buffer = BashPreviewBuffer(
                    max_lines=_BASH_PROGRESS_PREVIEW_MAX_LINES,
                    max_line_chars=_BASH_PROGRESS_PREVIEW_MAX_LINE_CHARS,
                    hidden_line_prefixes=(cwd_marker, marker),
                )
                output_chars_seen = 0
                exit_code = "unknown"
                ...
                def append_output(text: str) -> None:
                    """Store full tool output and independently advance the bounded UI view."""
                    nonlocal output_chars_seen
                    if not text:
                        return
                    output_parts.append(text)
                    preview_buffer.append(text)
                ...
                async def emit_progress(force: bool = False):
                    ...
                    if progress_callback is None:
                        return
                    snapshot = preview_buffer.snapshot()
                    if snapshot is None:
                        return
                    now = time.monotonic()
                    grew = output_chars_seen - progress_chars_at_emit
                    ...
                    try:
                        ...
                        progress_chars_at_emit = output_chars_seen
                    except asyncio.CancelledError:
                        raise
                ...
                async def read_until_marker():
                    ...
                    while True:
                        ...
                        if marker_pos >= 0:
                            ...
                            if marker_pos:
                                append_output(pending[:marker_pos])
                        ...
                        if len(pending) > keep_tail:
                            append_output(pending[:-keep_tail])
