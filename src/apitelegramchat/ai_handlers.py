...
<ul>
  ...
  <li><b>填写意图描述（_description，强制）。</b> 凡是工具参数中声明了 <code>_description</code> 的工具（bash、text_editor、route、weather、book_lookup、exchange_rate、crypto_price、distance、POI 检索等），<b>每次调用都必须如实填写</b>：用一句话（不超过 60 字、与用户语言一致）说明本次操作的目的。该描述会作为执行进度实时展示给用户——漏填或留空的 bash 调用会被参数校验拒绝，需补上后重新发起。</li>
