...
# 修复：原 DEFAULT_MAX_MESSAGES / DEFAULT_MAX_CHARS = None 表示"无上限"，
...
#   * DEFAULT_MAX_CHARS：估算字符数上限 200k（粗略对应 ~50k tokens）
# 这些值仍很宽松，留有充足的上下文窗口，但能在意外长会话上踩刹车。
DEFAULT_MAX_MESSAGES = int(os.getenv("CONTEXT_MAX_MESSAGES", "50"))
DEFAULT_MAX_CHARS = int(os.getenv("CONTEXT_MAX_CHARS", str(200_000)))
...
def select_request_context(
    ...
    *,
    ...
    ``max_messages`` (default DEFAULT_MAX_MESSAGES) and ``max_chars``
    (default DEFAULT_MAX_CHARS) — both can be configured per-call.
    """
    ...
    for message in reversed(history):
        ...
        if max_messages and len(selected_reversed) >= max_messages:
            break
        if max_chars and used + size > max_chars and selected_reversed:
            # 这条加进去会超字符上限，且至少已有一条入选，停在这里。
            break
