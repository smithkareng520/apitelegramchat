...
FORBIDDEN_TOOLS = {"subagent", "memory", "skill", "message_user"}
# 注意：todo / bash / text_editor 仍允许，因为子 agent 可能需要查 / 写工作区文件。
# message_user 不允许由子 agent 调用，否则会让父 agent 陷入不可控的嵌套人工等待。
...
- 禁止调用 subagent 工具（不能递归派生子 agent），也不能调用 message_user（人工交互必须由父 agent 负责）。
