...
里暴露（模型通过 send 布尔参数决定是否发送，默认 false，因此 /show on
下模型看不到该工具也就不会产生除草稿外的单独发送）；非静默回合除了不
提供工具定义，出站历史副本中已有的调用痕迹（assistant 的 tool_calls 与
配对的 tool 消息）也一并拔除，避免模型看到并模仿调用一个当前不可用的
工具；回到静默回合时痕迹在原位置原样插回（持久历史从不被改动，插拔只
作用于出站副本）。
