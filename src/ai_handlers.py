...
<ul>
  ...
  <li><b>填写意图描述（仅 bash 工具）。</b> 只有 <code>bash</code> 声明了 <code>_description</code> 参数（<b>必填</b>）：每次调用必须用一句话（不超过 60 字、与用户语言一致）说明本次命令的目的，该描述会作为执行进度实时展示给用户——漏填或留空的 bash 调用会被参数校验拒绝，需补上后重新发起。其余所有工具（text_editor、web_search、天气、地图等）均<b>不声明</b>该参数，无需也不应传入。</li>
