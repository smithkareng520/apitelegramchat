...
# 有对应工具的模型看到工具 schema 自然会发起真正的 tool_calls；没有
# 这些工具的模型会把它理解为"从上下文里回顾任务与记忆"，不会在正文
# 里幻觉出 {"tool_calls": ...} 这样的文本。始终存在的工具
# （message_user / deliver_reply / web_search）才保留具名指引。
WAKEUP_PROMPT = """
[系统后台唤醒｜TIMER 主动巡检回合]

你现在处于"主动巡检回合"。这不是一次普通的聊天请求，而是一次由你主动
发起对话、回顾近况、看看有什么值得跟进或分享的机会。系统已另行告知
你当前草稿预览的开关状态（/show）：开启时你的过程与最终回复对用户可见；
静默时则由你
...
1. 看看有什么值得跟进的（第一优先级）
   - 了解一下近况：可以先查看任务清单（Todo）、长期记忆（Memory），
     再结合最近对话；关注已到期、临近到期、长期未推进、现在适合推进、
     或现在值得提醒的事。
   - 事情明确且安全、且无需用户补充信息时，可以顺手推进；否则提醒用户。
