...
# ========== packaged skills R2 bundle ==========
# 系统 Skills 只以一个 ZIP bundle 存储在 R2。workspace/skills 是运行时副本，
# 用户在运行期创建/修改的技能不再逐文件写入 R2；下次部署恢复的是项目源码
# 打包生成的 bundle。这样 R2 的 skills 域永远只有一个对象，部署/同步时只需
# 一次下载 + 一次解压（进程内所有 workspace 共用解压结果）。
