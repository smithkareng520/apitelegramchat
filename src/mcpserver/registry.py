...
async def web_search(_: MCPRequestContext, args: JsonObject) -> str:
    from search_engine import execute_web_search
    return await invoke(
        execute_web_search,
        args["query"],
...
async def exchange_rate(_: MCPRequestContext, args: JsonObject) -> str:
    from search_engine import execute_exchange_rate
...
async def route(_: MCPRequestContext, args: JsonObject) -> str:
    from search_engine import execute_route
    return await invoke(
        execute_route,
        args["origin"],
        args["destination"],
...
async def distance(_: MCPRequestContext, args: JsonObject) -> str:
    from search_engine import execute_distance
...
async def poi_keyword(_: MCPRequestContext, args: JsonObject) -> str:
    from search_engine import execute_keyword_search
...
async def poi_nearby(_: MCPRequestContext, args: JsonObject) -> str:
    from search_engine import execute_nearby_search
...
async def present_files(context: MCPRequestContext, args: JsonObject) -> str:
    from tool_executors import execute_present_files
    return await invoke(
...
READ_ONLY_SPECS: tuple[ToolSpec, ...] = (
        ...
        object_schema(
            {
                ...
                "num_results": int_field("Optional result count (1-50). If omitted, returns a default of 10.", 1, 50),
                "offset": int_field("Optional zero-based result offset for pagination.", 0),
            },
            ("query",),
        ),
        web_search,
    ...
    ToolSpec("workspace.view", "View text file or directory", "View a private UTF-8 text file with 1-based line numbers, or list directory contents up to 2 levels deep.", object_schema({"command": {"type": "string", "enum": ["view"]}, "path": text_field("Relative workspace file or directory path (or '.' for root).", 1), "view_range": {"type": "array", "items": {"type": "integer"}, "minItems": 2, "maxItems": 2}}, ("command", "path")), workspace_view),
)
...
MUTATION_SPECS: tuple[ToolSpec, ...] = (
    ...
    ToolSpec("workspace.edit", "Edit text file", "Create or update a private UTF-8 text file. Explicit deployment opt-in required.", object_schema({"command": {"type": "string", "enum": ["create", "str_replace", "insert"]}, "path": text_field("Relative workspace file path.", 1), "old_str": text_field("Exact existing text; it must occur exactly once."), "new_str": text_field("Replacement text (or text to insert)."), "insert_line": int_field("Insert after 1-based line; use 0 for the start.", 0), "insert_text": text_field("Text to insert."), "file_text": text_field("Complete initial content for a new file.")}, ("command", "path")), workspace_edit),
    ToolSpec("workspace.present", "Present workspace files", "Present selected files from the upload/ staging directory using workspace-relative paths such as upload/report.pdf. Paths must resolve inside upload/. Explicit deployment opt-in required.", object_schema({"paths": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 20}}, ("paths",)), present_files),
)
