...
MUTATION_SPECS: tuple[ToolSpec, ...] = (
    ...
    ToolSpec("workspace.present", "Present staged files", "Present selected staged files using workspace-relative paths such as upload/report.pdf. Files must be staged under upload/. Explicit deployment opt-in required.", object_schema({"paths": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 20}}, ("paths",)), present_files),
)
