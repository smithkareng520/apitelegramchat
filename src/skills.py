...
async def start_packaged_skill_auto_sync() -> None:
    ...
    # Critical path: hydrate existing workspaces from the R2 skills snapshot
    ...
    # Then refresh packaged skills. Any package refresh that changes a workspace
    # is picked up by the R2 watcher below and becomes the new snapshot.
    ...
    # A separate lightweight watcher persists runtime workspace/skills changes.
    # It uses path/size/mtime metadata only; no content hash is calculated.
    global _workspace_r2_watcher_task, _workspace_r2_watcher_stop
...
async def stop_packaged_skill_auto_sync() -> None:
    ...
    # Snapshot handles before changing module-level state.
    stop_event = _sync_watcher_stop
    task = _sync_watcher_task
    r2_stop = _workspace_r2_watcher_stop
    r2_task = _workspace_r2_watcher_task

    if stop_event is not None:
        stop_event.set()
    if r2_stop is not None:
        r2_stop.set()

    for watcher in (task, r2_task):
        if watcher is None:
            continue
        watcher.cancel()
        try:
