...
FORBIDDEN_TOOLS = {"subagent", "memory", "message_user", "deliver_reply"}
...
- 专心完成本任务，不要扩展话题。
- 可以调用提供的工具来获取信息或操作文件。
- 工具参数中声明了 _description 的工具（bash、route、weather、exchange_rate、book_lookup、crypto_price、distance、POI 检索、todo 等），每次调用都必须填写该参数：一句话（≤60字）说明本次操作目的，它会作为执行进度展示给用户。bash 的 _description 为必填，漏填或留空会被参数校验拒绝，需补上后重新发起。text_editor、fetch_url、wikipedia、news、qr_code、present_files 不声明该参数，无需填写。
...
def _truncate(s: str, token_budget: int = SUBAGENT_TOOL_RESULT_TOKEN_BUDGET, fn_name: str | None = None) -> str:
    ...
    if fn_name == "bash":
        from token_budget import truncate_to_token_budget_head_tail
